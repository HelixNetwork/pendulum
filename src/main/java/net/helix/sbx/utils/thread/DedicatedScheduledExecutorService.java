package net.helix.sbx.utils.thread;
 ...
 * <br />
 * It informs the user about its lifecycle using the logback loggers used by SBX. In addition it offers "silent" methods
 * of the {@link ScheduledExecutorService} that do not throw {@link Exception}s when we try to start the same task
 * multiple times. This is handy for implementing the "start" and "shutdown" methods of the background workers of SBX
...
public class DedicatedScheduledExecutorService extends BoundedScheduledExecutorService {
     ...
     * <br />
     * It informs the user about its lifecycle using the logback loggers used by SBX. In addition it offers "silent"
     ...
     * workers of SBX that would otherwise have to take care of not starting the same task more than once (when trying
     ...
     * <br />
     * Note: This is for example used by the {@link net.helix.sbx.utils.log.interval.IntervalLogger} which does not want
